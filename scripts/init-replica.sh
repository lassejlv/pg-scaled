...
PGPASSWORD=repl123 pg_basebackup -h $POSTGRES_PRIMARY_HOST -p $POSTGRES_PRIMARY_PORT -U replicator -D /var/lib/postgresql/data -P -W -R
...
primary_conninfo = 'host=$POSTGRES_PRIMARY_HOST port=$POSTGRES_PRIMARY_PORT user=replicator password=repl123 application_name=$(hostname)'
